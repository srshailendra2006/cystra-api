 ...
 *     responses:
 *       200:
 *         description: Party categories list
 *       401:
...
router.get('/', authenticate, async (req, res) => {
  try {
    ...
    const result = await pool.request()
      ...
      .query(`
        SELECT
          PartyCategoryID as party_category_id,
          CategoryCode as category_code,
          CategoryName as category_name,
          IsActive as is_active
        FROM dbo.PartyCategory
        WHERE (@active IS NULL OR IsActive = @active)
        ORDER BY CategoryName ASC, CategoryCode ASC
      `);
    ...
    return res.status(500).json({
      status: 'error',
      message: error.message || 'Failed to fetch party categories'
    });
  }
});
